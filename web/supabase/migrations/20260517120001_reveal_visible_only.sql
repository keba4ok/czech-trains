-- Defense in depth: reveal_challenge now refuses to reveal a challenge
-- that hasn't been drawn from the deck yet. The map already filters
-- visible=false markers out for players, but this stops anyone calling
-- the RPC directly from leaking title/description.
...
begin
  ...
  if not v_challenge.visible then
    raise exception 'challenge is not in the deck yet' using errcode = '22023';
  end if;
